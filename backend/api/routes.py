...
def create_app(config: Dict[str, Any] = None) -> Flask:
    ...
    def analyze_lightcurve():
        ...
        try:
            ...
            # Map window predictions back to original points using majority voting
    ...
    def list_available_models():
        ...
        from backend.ml.model_registry import list_models, get_display_name
        models = list_models(include_deep=False)
        return jsonify({
            'models': [
                {'id': m, 'name': get_display_name(m)} for m in models
            ]
        })
    ...
    def evaluate_model():
        ...
        Expects:
            - file: Light curve file with 'label' column
        ...
        try:
            ...
            if 'label' not in df.columns:
                temp_path.unlink()
                return jsonify({'error': 'File must have a "label" column for evaluation'}), 400
