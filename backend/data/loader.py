...
class LightCurveLoader:
    ...
    def load_from_lightkurve(self, lc) -> pd.DataFrame:
        """
        Convert a lightkurve LightCurve object to standardized DataFrame.
        ...
        if hasattr(lc, 'meta') and lc.meta:
            self.metadata = {
                'object_name': lc.meta.get('OBJECT', lc.meta.get('TARGETID', 'Unknown')),
                'telescope': lc.meta.get('TELESCOP', 'Unknown'),
                'instrument': lc.meta.get('INSTRUME', 'Unknown'),
            }
