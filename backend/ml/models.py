...
class AnomalyDetector:
    ...
    def predict(self, features: np.ndarray, method: str = 'ensemble') -> np.ndarray:
        ...
        if method == 'isolation_forest':
            ...
            # Anomaly only if BOTH methods agree (-1 in sklearn convention)
...
def map_window_predictions_to_points(predictions, scores, n_points, window_size, stride,
                                     method='vote'):
    """
    Map window-level predictions back to individual data points.

    Args:
        predictions: Window predictions array (-1 for anomaly, 1 for normal)
        scores: Window anomaly scores (more negative = more anomalous)
        n_points: Total number of data points in the original light curve
        window_size: Size of each sliding window
        stride: Stride between consecutive windows
        method: Mapping strategy
            'vote' - majority voting: point is anomalous if >50% of its windows are anomalous
            'any' - legacy behavior: point is anomalous if ANY window containing it is anomalous
            'score' - use mean anomaly score per point with automatic thresholding

    Returns:
        anomaly_mask: Boolean array of shape (n_points,)
        point_scores: Float array of shape (n_points,) with per-point anomaly scores
    """
    anomaly_votes = np.zeros(n_points, dtype=float)
    vote_counts = np.zeros(n_points, dtype=float)
    score_sums = np.zeros(n_points, dtype=float)

    for i, (pred, score) in enumerate(zip(predictions, scores)):
        start_idx = i * stride
        end_idx = min(start_idx + window_size, n_points)

        vote_counts[start_idx:end_idx] += 1
        score_sums[start_idx:end_idx] += score

        if pred == -1:
            anomaly_votes[start_idx:end_idx] += 1

    # Avoid division by zero for points not covered by any window
    vote_counts = np.maximum(vote_counts, 1)
    point_scores = score_sums / vote_counts

    if method == 'vote':
        # Majority voting: anomalous if >50% of overlapping windows flag it
        anomaly_fractions = anomaly_votes / vote_counts
        anomaly_mask = anomaly_fractions > 0.5
    elif method == 'any':
        # Legacy behavior: anomalous if any window flags it
        anomaly_mask = anomaly_votes > 0
    elif method == 'score':
        # Score-based: use mean score with percentile threshold
        threshold = np.percentile(point_scores, 10)  # bottom 10% scores
        anomaly_mask = point_scores < threshold
    else:
        raise ValueError(f"Unknown method: {method}. Use 'vote', 'any', or 'score'.")

    return anomaly_mask, point_scores
